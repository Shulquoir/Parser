...
begin
  ...
  FForm := TMainForm.Create(Application); // Створюємо екземпляр форми
...
begin
  ...
  FForm.OpenedDFNToModFilePath := 'D:\Programs\Embarcadero\ТЗ\16.dfn';
  ...
  FForm.OriginalDFNToModFileContent.Add('ID:456' + #4 + 'Orig:''Значения''' + #4 + 'Curr:''Значения''');
...
begin
  ...
  // Викликаємо процедуру, яку хочемо протестувати
  FForm.odOpenDFNToMod.FileName := TestFileName; // Симулюємо натискання кнопки та передаємо ім'я тестового файлу
...
var
  Key: Char;
begin
  FForm.ePathDFNToMod.Text := 'D:\Programs\Embarcadero\ТЗ\Unit112.tsv';
  Key := #13;
