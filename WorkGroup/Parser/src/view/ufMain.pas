...
type
  ...
  private
    ...
    const STR_FIRST_OPEN_TSV_AND_DFN_FILES = 'Сперва откройте TSV и DFN файлы с совпадающими именами!';
    const STR_OPEN_FILES_WITH_MATCHING_NAME = 'Пожалуйста, откройте TSV и DFN файлы с совпадающими именами.';
    const SRT_SUCCESSFUL_REWRITE_OF_DFN_FILE = 'Файл DFN был перезаписан.';
...
begin
  ...
  sdSaveTSV.FileName := ChangeFileExt(ExtractFileName(OpenedDFNFilePath), STR_TSV_EXTENSION); // Отримуємо
                  // пропоноване ім'я файлу для зберігання, через ім'я відкритого файлу із заміною формата
  if sdSaveTSV.Execute = false then
    Exit;

  try
    OutputFileContent := TStringList.Create;
    IsFileCreate := TParsing.ConvertDFNToTSV(OpenedDFNFilePath, OutputFileContent); // Створюємо TSV файл
...
begin
  ...
  begin
    MessageDlg(STR_FIRST_OPEN_TSV_AND_DFN_FILES, mtInformation, [mbOK], 0); // Виведення повідомленн, якщо не було
  ...
  TParsing.ModifyDFN(OpenedTSVFilePath, OpenedDFNToModFilePath); // Перезапис відкритого DFN файлу
...
/// <summary>
/// Застосовує шлях до файлу та зберігає його в глобальній змінній.
/// </summary>
/// <param name="filePath">Шлях до файлу, який потрібно застосувати.</param>
/// <param name="globalFilePath">Глобальна змінна, в яку буде збережено шлях до файлу.</param>
/// <param name="fileType">Тип файлу (наприклад, "TSV" або "DFN").</param>
...
/// <summary>
/// Відкриває файл у вказаній програмі за її назвою та шляхом до файлу.
/// </summary>
/// <param name="programName">Назва програми, у якій потрібно відкрити файл (наприклад, "Excel" або "Calc").</param>
/// <returns>True, якщо файл був успішно відкрито; False, якщо програмі треба вибрати відповідний виконуваний файл.</returns>
